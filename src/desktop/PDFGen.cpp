...
static int pdf_save_object_stream(struct pdf_doc *pdf, std::string &out, int index) {
  ...
  case OBJ_page: {
    ...
    out += "  /Resources <<\r\n";
    out += "    /Font <<\r\n";
    for (struct pdf_object *font = pdf_find_first_object(pdf, OBJ_font); font; font = font->next) {
    ...
    for (struct pdf_object *image = pdf_find_first_object(pdf, OBJ_image); image; image = image->next) {
    ...
    for (int i = 0; i < flexarray_size(&object->page.children); i++) {
      struct pdf_object *child = (struct pdf_object *)flexarray_get(&object->page.children, i);
    ...
    if (flexarray_size(&object->page.annotations)) {
      out += "  /Annots [\r\n";
      for (int i = 0; i < flexarray_size(&object->page.annotations); i++) {
        struct pdf_object *child = (struct pdf_object *)flexarray_get(&object->page.annotations, i);
        out += std::format("{:d} 0 R\r\n", child->index);
      }
      out += "]\r\n";
    }
  ...
  case OBJ_bookmark: {
    ...
    for (other = object->prev; other && other->bookmark.parent != object->bookmark.parent; other = other->prev)
      ;
    if (other) {
      out += std::format("  /Prev {:d} 0 R\r\n", other->index);
    }
    // Find the next bookmark with the same parent
    for (other = object->next; other && other->bookmark.parent != object->bookmark.parent; other = other->next)
      ;
    if (other) {
  ...
  case OBJ_pages: {
    ...
    out += "<<\r\n"
           "  /Type /Pages\r\n"
           "  /Kids [ ";
    for (struct pdf_object *page = pdf_find_first_object(pdf, OBJ_page); page; page = page->next) {
      npages++;
